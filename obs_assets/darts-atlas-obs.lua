...
--  local match_calendar = obs.obs_get_source_by_name("[DA] Calendar")
  ...
  local match_order_of_merit = obs.obs_get_source_by_name("[DA] Order of Merit")
  local match_preview = obs.obs_get_source_by_name("[DA] Match Preview")
  local match_round = obs.obs_get_source_by_name("[DA] Match Round")
  ...
  local match_headshot_p1 = obs.obs_get_source_by_name("[DA] Headshot - P1")
  local match_headshot_p2 = obs.obs_get_source_by_name("[DA] Headshot - P2")
  local match_media_shot_p1 = obs.obs_get_source_by_name("[DA] Media Shot - P1")
  local match_media_shot_p2 = obs.obs_get_source_by_name("[DA] Media Shot - P2")
...
--  local match_calendar_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_calendar")
  ...
  local match_headshot_p1_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_headshot?player=1")
  local match_headshot_p2_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_headshot?player=2")
  local match_media_shot_p1_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_media_shot?player=1")
  local match_media_shot_p2_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_media_shot?player=2")
--   local match_schedule_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_schedule")
  local match_order_of_merit_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_order_of_merit")
  local match_round_url = (tostring(base_url) .. tostring(match_hash_id) .. "/broadcast_round")
...
--  obs.obs_source_release(match_calendar)
  ...
  -- Headshots
  obs.obs_data_set_string(settings, "url", match_headshot_p1_url)
  obs.obs_source_update(match_headshot_p1, settings)
  obs.obs_source_release(match_headshot_p1)
  obs.obs_data_set_string(settings, "url", match_headshot_p2_url)
  obs.obs_source_update(match_headshot_p2, settings)
  obs.obs_source_release(match_headshot_p2)

-- Media Shots
obs.obs_data_set_string(settings, "url", match_media_shot_p1_url)
obs.obs_source_update(match_media_shot_p1, settings)
obs.obs_source_release(match_media_shot_p1)
obs.obs_data_set_string(settings, "url", match_media_shot_p2_url)
obs.obs_source_update(match_media_shot_p2, settings)
obs.obs_source_release(match_media_shot_p2)

obs.obs_data_set_string(settings, "url", match_round_url)
obs.obs_source_update(match_round, settings)
obs.obs_source_release(match_round)

obs.obs_data_set_string(settings, "url", match_order_of_merit_url)
obs.obs_source_update(match_order_of_merit, settings)
obs.obs_source_release(match_order_of_merit)
